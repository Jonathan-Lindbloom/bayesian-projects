...
from loss_function import batch_eval_loss
...
def plot_random_comparison(w_opt, ending_returns, num_compare=10000):
    ...
    losses = batch_eval_loss(rand_weights, ending_returns, lam=2.0)
    opt_port_loss = batch_eval_loss(w_opt, ending_returns, lam=2.0)
    ...
    # Loss function evals
    axs[2].hist(losses, bins=30)
    axs[2].axvline(opt_port_loss, color="red", label="Opt = {:.1f}%".format(opt_port_loss))
    axs[2].set_title("Portfolio Loss")
    axs[2].legend()
